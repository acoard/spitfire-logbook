...
const MapViewToggle: React.FC<MapViewToggleProps> = ({
  ...
  return (
    <div className="h-full w-full relative">
      {/* View Toggle Controls - z-index below modals (modals typically 1000+) */}
      <div className="absolute top-4 left-4 z-[50] flex">
        <div className="bg-stone-900/95 backdrop-blur-sm rounded-lg border-2 border-amber-700/40 shadow-xl overflow-visible flex">
          {/* 2D Map Button */}
          <button
          ...
          {/* 3D Globe Button with WIP stamp */}
          <button
            onClick={() => handleViewChange('globe')}
            className={`
              relative flex items-center gap-2 px-4 py-2.5 font-typewriter text-xs uppercase tracking-wider
            ...
            `}
            title="Switch to 3D Globe View (Experimental)"
          >
            <Globe2 className="w-4 h-4" />
            <span className="hidden sm:inline">Globe (3D)</span>
            {/* WIP Stamp overlay */}
            <span className="absolute -top-2 -right-2 bg-red-600 text-white text-[8px] font-bold px-1.5 py-0.5 rounded transform rotate-12 shadow-md border border-red-400">
              WIP
            </span>
          </button>
        </div>
